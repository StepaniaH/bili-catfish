...
describe('applyOverlay', () => {
  it('adds mask with video label and unblock button', () => {
    ...
    applyOverlay(el, { videoHit: true, uperHit: false, adHit: false, onUnblockVideo, onUnblockUper: vi.fn() });
  ...
  it('shows combined label and both buttons when both rules hit', () => {
    const el = card();
    applyOverlay(el, { videoHit: true, uperHit: true, adHit: false, onUnblockVideo: vi.fn(), onUnblockUper: vi.fn() });
  ...
  it('shows only uper label when only uper rule hit', () => {
    const el = card();
    applyOverlay(el, { videoHit: false, uperHit: true, adHit: false, onUnblockVideo: vi.fn(), onUnblockUper: vi.fn() });
  ...
  it('is idempotent: re-apply does not duplicate mask', () => {
    const el = card();
    applyOverlay(el, { videoHit: true, uperHit: false, adHit: false, onUnblockVideo: vi.fn(), onUnblockUper: vi.fn() });
    applyOverlay(el, { videoHit: true, uperHit: false, adHit: false, onUnblockVideo: vi.fn(), onUnblockUper: vi.fn() });
  ...
  it('shows ad label without buttons when only ad hit', () => {
    const el = card();
    applyOverlay(el, { videoHit: false, uperHit: false, adHit: true, onUnblockVideo: vi.fn(), onUnblockUper: vi.fn() });
  ...
  it('combines ad label with rule labels', () => {
    const el = card();
    applyOverlay(el, { videoHit: true, uperHit: true, adHit: true, onUnblockVideo: vi.fn(), onUnblockUper: vi.fn() });
