...
describe('applyOverlay', () => {
  it('adds mask with video label and unblock button', () => {
    ...
    applyOverlay(el, { videoHit: true, uperHit: false, onUnblockVideo, onUnblockUper: vi.fn() });
  ...
  it('shows combined label and both buttons when both rules hit', () => {
    const el = card();
    applyOverlay(el, { videoHit: true, uperHit: true, onUnblockVideo: vi.fn(), onUnblockUper: vi.fn() });
  ...
  it('shows only uper label when only uper rule hit', () => {
    const el = card();
    applyOverlay(el, { videoHit: false, uperHit: true, onUnblockVideo: vi.fn(), onUnblockUper: vi.fn() });
  ...
  it('is idempotent: re-apply does not duplicate mask', () => {
    const el = card();
    applyOverlay(el, { videoHit: true, uperHit: false, onUnblockVideo: vi.fn(), onUnblockUper: vi.fn() });
    applyOverlay(el, { videoHit: true, uperHit: false, onUnblockVideo: vi.fn(), onUnblockUper: vi.fn() });
