...
function labelFor(videoHit: boolean, uperHit: boolean, adHit: boolean, promoHit: boolean, courseHit: boolean, categoryHit: boolean, categoryName?: string): string {
  const parts: string[] = [];
  if (videoHit) parts.push('已屏蔽该视频');
  if (uperHit) parts.push('已屏蔽该 UP 主');
  if (adHit) parts.push('已屏蔽广告');
  if (promoHit) parts.push('已屏蔽推广');
  if (courseHit) parts.push('已屏蔽课堂');
...
export function applyOverlay(card: Element, opts: ApplyOptions): void {
  ...
  const label = labelFor(opts.videoHit, opts.uperHit, opts.adHit, opts.promoHit, opts.courseHit, opts.categoryHit, opts.categoryName);
